...
- un_all_v74.py: 移除 --run-config-file CLI 與下游參數傳遞。
- un_locked_test_v74.py: 移除 --run-config-file CLI 與傳給 	rain_v74.py 的外部 config 參數。
- sync_all_outputs_v74.py: 移除外部 config JSON 的同步清單。
- 刪除 root configs/ 內的外部 JSON 設定檔與 ll/configs/ 舊同步資料。

**修改內容**:
- 保留現有 15 組 RUN_CONFIGS 與 5 組 ABLATION_RUN_CONFIGS，包含目前 Task1/Task2/Task3 missing augmentation 與 bg_borderline 設定。
- ecommended、blation、ll 三種 preset 仍可使用；ll 仍代表 20 組 config。
...
**修改目的與意義**：依使用者確認後的臨床規則，將 Task2/Task3 從單純 complete/incomplete gating 收斂為 rule evidence score。保留 ule_forced 的硬判 label 作為 baseline，同時讓正式 ule_abstain 與 hybrid_rule_first 依 score 判斷 rule 是否可信。

**修改檔案與範圍**：
- clinical_rules_v74.py：Task2 ABG clear/borderline 規則、Task2 evidence score、Task3 Vea/peak/compliance/width evidence score。
- preprocessing_v74.py：補齊 ule_evidence_score、score_deductions 輸出欄位。
- hybrid_evaluation_v74.py：hybrid rule-first 改用 score gating，並保留新版 decision reason。
- error_analysis_v74.py：rule 欄位清單補 ule_evidence_score 與 score_deductions。
- dashboard_three_tasks_metaIRL_v74.py：dashboard 即時 hybrid decision 改用 score gating。

**修改內容**：
- Task2 clear ABG 改為 bs(AC-BC) >= 10 dB。
- Task2 borderline ABG 改為 8 <= abs(AC-BC) < 10 dB，不直接觸發 CHL/MHL，只扣 score 並加 warning。
...
- Hybrid 使用 rule 的條件改為 rule label 存在且 ule_confidence >= threshold，不再強制要求 complete_for_rule=True 與 aseline_covered=True。

**修改前後差異**：
- 修改前：hybrid rule-first 主要受 complete_for_rule / aseline_covered 二分法限制；Task2 ABG=10 被視為 borderline；Task3 未將 Vea 缺失納入 rule evidence score。
- 修改後：hybrid rule-first 以 score 控制；Task2 ABG=10 視為 clear ABG；Task3 Vea 缺失會扣分與 warning，但不直接改 A/B/C label。

**新增或改善功能**：
- 新增 ule_evidence_score 與 score_deductions 欄位。
- 新增 score-based hybrid_decision_reason：ule_score_ge_threshold、ule_score_ge_threshold_with_warning、model_fallback_low_rule_score、model_fallback_no_rule_prediction。
...
**後續優化**：Medium：目前暫存檢查顯示 Task2 在 ABG>=10 搭配「任一頻率 clear ABG 即判 CHL/MHL」後，rule-forced 表現明顯下降；若要改善臨床一致性，建議下一輪討論是否加入「至少兩個非 censored ABG 頻率」或其他圖形判讀條件。
## 2026-07-02 輔助流程 Score Gating 接口補齊

**修改時間**：2026-07-02 16:09:56

**修改目的與意義**：P7 總體接口檢查時發現 rtificial_missingness_v74.py 與 model_profile_v74.py 仍保留舊式 complete_for_rule & baseline_covered rule gating。為避免不同流程對 hybrid rule-first 的定義不一致，補齊為 score-based gating。

**修改檔案與範圍**：
- rtificial_missingness_v74.py：strategy_predictions() 中的 use_rule 條件。
- model_profile_v74.py：hybrid latency profile 中 rule/model selection 條件。

**修改內容**：
- 使用 rule label 是否存在與 ule_confidence >= threshold 決定是否採用 rule。
- 不再強制要求 complete_for_rule=True 與 aseline_covered=True 才能採用 rule。
...
- 已重跑正式 aseline_rules_v74.py、hybrid_evaluation_v74.py --mode both、error_analysis_v74.py --mode both。
